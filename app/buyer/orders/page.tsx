...
import { useBuyerOrder } from '@/api/orderApi';
import { useStateContext } from '@/context/StateContext';
import { BuyerOrderType } from '@/utils/type';
import Link from 'next/link';
import React, { useEffect, useState } from 'react'

const Page = () => {
  const [orders,setOrder] = useState<BuyerOrderType[]>([]);
  const {userInfo} = useStateContext()
  const {buyerOrder,isLoading} = useBuyerOrder();
  

  useEffect(()=>{
     if(buyerOrder){
      setOrder(buyerOrder?.message)
     }
  },[userInfo,buyerOrder])
        ...
        <tbody>
          {
            orders.map((order) => (
              <tr className="bg-white border-b  hover:bg-gray-50" key={order.id}>
                ...
                <td className="px-6 py-4">
                  <Link href={`/buyer/orders/messages/${order.id}?recipentId=${order.gig.userId}`}
